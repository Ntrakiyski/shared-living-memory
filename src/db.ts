...
type MetadataIndex = { property: string; type: string };

function metadataIndexes(value: unknown, found: MetadataIndex[] = []): MetadataIndex[] {
  if (Array.isArray(value)) {
    for (const item of value) metadataIndexes(item, found);
    return found;
  }
  if (!value || typeof value !== "object") return found;
  const record = value as Record<string, unknown>;
  const normalized = new Map(
    Object.entries(record).map(([key, item]) => [key.replaceAll("_", "").toLowerCase(), item]),
  );
  ...
  for (const item of Object.values(record)) metadataIndexes(item, found);
  return found;
}

export function assertVectorizeMetadataIndexes(output: string): void {
  let parsed: unknown;
  try {
    parsed = JSON.parse(output);
  } catch {
    throw new Error("vector_metadata_indexes_invalid:json_invalid");
  }
  const indexes = metadataIndexes(parsed);
  const required = [
    ["owner_user_id", "string"],
    ["is_private", "boolean"],
  ] as const;
  const failures: string[] = [];
  for (const [property, type] of required) {
  ...
  if (failures.length) {
    throw new Error(`vector_metadata_indexes_invalid:${failures.join(",")}`);
  }
}

export type KnownSemanticCanaryResult = {
  ok: boolean;
  code: "ok" | "semantic_canary_zero_results" | "semantic_canary_target_missing";
  match_count: number;
};

export function evaluateKnownSemanticCanary(
  matches: { id: string; score?: number; metadata?: Record<string, unknown> }[],
  expectedEntryId: string,
): KnownSemanticCanaryResult {
  if (matches.length === 0) {
    return { ok: false, code: "semantic_canary_zero_results", match_count: 0 };
  }
  const found = matches.some(match =>
    (match.metadata?.parentId ?? match.id) === expectedEntryId,
  );
  return found
    ? { ok: true, code: "ok", match_count: matches.length }
    : { ok: false, code: "semantic_canary_target_missing", match_count: matches.length };
}
